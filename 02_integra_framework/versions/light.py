...
class INTEGRALight:
    ...
    def process_request(
        ...
        try:
            ...
            print(
                f"✅ Anfrage erfolgreich verarbeitet (Path: {result.processing_path}, "
                f"Score: {result.align_score:.2f})"
            )
    ...
    def _execute_processing_path(self, processing_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if path == 'fast_path':
            ...
            try:
                ...
                processing_context['context']['decision'] = {
                    'response': (
                        "Entschuldigung, eine umfassende ethische Analyse ist derzeit nicht verfügbar. "
                        "Bitte kontaktieren Sie einen menschlichen Ansprechpartner."
                    ),
                    'path_taken': 'fallback',
                    'confidence': 0.1
                }
            ...
            processing_context['context']['decision'] = {
                'response': (
                    "Entschuldigung, ich kann Ihre Anfrage derzeit nicht verarbeiten. "
    ...
    def _compile_final_result(self, processing_context: Dict[str, Any], start_time: datetime) -> ProcessingResult:
        ...
        escalation_triggered = (
            context.get('auto_escalation_triggered', False) or
            context.get('escalation_recommended', False)
        )
    ...
    def _create_error_result(
        ...
        return ProcessingResult(
            response=(
                f"Entschuldigung, es ist ein Fehler aufgetreten: {error_message}. "
...
def run_unit_tests():
    ...
    def test_error_handling():
        ...
        assert not invalid_update['success']
...
def run_demo():
    ...
    for domain, description in domains:
        ...
        align_w = integra.config.align_weights
        print(
            f"⚖️ ALIGN-Gewichte: Integrity={align_w['integrity']:.1f}, "
            f"Nurturing={align_w['nurturing']:.1f}"
        )
    ...
    print("🤔 Normale Anfrage: Wetter-Frage")
